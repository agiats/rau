...
class ParseTree(Tree):
    def __init__(self, symbol, children=None, rule=None):
        ...
        if rule is not None:
            if not (
                rule.left == symbol
                and all(x == y.symbol for (x, y) in zip(rule.right, self.children))
            ):
    ...
    def probability(self):
        if self.rule is None:
            ...
            return self.rule.probability * product(c.probability for c in self.children)
    ...
    def log_probability(self):
        if self.rule is None:
            ...
            return self.rule.log_probability + sum(
                c.log_probability for c in self.children
            )
