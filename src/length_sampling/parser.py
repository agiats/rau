...
from .parse_tree import left_parse_to_tree
from .pdt import Transducer, Transition
from .transducer import TransducerIndex, simulate_transducer, initial_step
...
def _grammar_to_transducer_transitions(grammar):
    yield Transition(
        state_from=START_STATE, state_to=LOOP_STATE, pushed_symbol=grammar.start
    )
    ...
    for rule in grammar.rules:
        ...
        for symbol in reversed(rule.right):
            yield Transition(state_from=state, state_to=state + 1, pushed_symbol=symbol)
            state += 1
        yield Transition(state_from=state, state_to=LOOP_STATE)
        state += 1
    ...
    def _enumerate_strings_from_symbol(symbol):
        if symbol.is_terminal:
            ...
            if rules is not None:
                for rule in rules:
                    choices = [
                        list(_enumerate_strings_from_symbol(s)) for s in rule.right
                    ]
