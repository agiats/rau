...
from .grammars.cfg import Grammar, Rule, Nonterminal, Terminal
...
class TransducerIndex:
    ...
    def __init__(self, transducer):
        ...
        self.scanning_pop = group_by(
            (
                t
                for t in transducer.transitions
...
def construct_grammar(transducer_index, Li, P):
    ...
    for U_record in Li:
        ...
        ((r, A, _), (q, B, j)) = U
        if (
            transducer_index.transducer.is_accept_state(r)
            and A == STACK_BOTTOM
            and q == q0
            and B == STACK_BOTTOM
            and j == 0
...
class PreNonScanningStep(Step):
    ...
    def simulate_non_scanning_transitions(self):
        ...
        simulate_non_scanning_transitions(
            self.transducer_index, self.i, self.Li, self.Ai, self.Ei, self.P, self.P_set
        )
        return PreScanningStep(
            self.transducer_index, self.i, self.Li, self.Ei, self.P, self.P_set
        )
...
class PreScanningStep(Step):
    ...
    def simulate_scanning_transitions(self, xi):
        ...
        Li, Ai, Ei = simulate_scanning_transitions(
            self.transducer_index, xi, self.i, self.Li, self.Ei, self.P, self.P_set
        )
