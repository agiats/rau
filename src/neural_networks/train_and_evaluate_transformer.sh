...
python "$RAU_DIR"/src/rau/tasks/language_modeling/train.py \
    ...
    --output "$RESULTS_DIR"/"$exp_name"/transformer/"$grammar_name"


eval_dir="$RESULTS_DIR"/"$exp_name"/transformer/"$grammar_name"/evaluation
mkdir -p "$eval_dir"
python "$RAU_DIR"/src/rau/tasks/language_modeling/evaluate.py \
    --load-model "$RESULTS_DIR"/"$exp_name"/transformer/"$grammar_name" \
    --training-data "$grammar_dir" \
    --input test \
