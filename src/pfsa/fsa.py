...
class PFSA:
    ...
    def MI(self, t: int) -> float:
        """Computes the mutual information I(Y_{<t}; Y_{>=t}).
        Args:
            t (int): The time t.
        Returns:
            float: The mutual information I(Y_{<t}; Y_{>=t}).
        """
        from numpy.linalg import matrix_power
        return H(self.λ.T * matrix_power(self.T, t - 1))

    def XXX(self, T: int = 100) -> float:
        """Computes (approximates up to time T) the value XXX.
        Args:
            T (int): The time T. Defaults to 100.
        Returns:
            float: The value XXX.
        """
        return sum(self.length_p(t) * self.MI(t) for t in tqdm(range(1, T), desc="Computing XXX"))
