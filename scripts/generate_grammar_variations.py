...
def main():
    parser = argparse.ArgumentParser(description="Generate grammar variations based on switch values")
    parser.add_argument("input_file", help="Path to input grammar file")
    parser.add_argument("output_dir", help="Directory to output generated grammars")
    parser.add_argument("-n", "--num_grammar", type=int, default=-1,
                        help="Number of grammars to generate. If -1, generate all possible combinations")

    args = parser.parse_args()

    generate_grammar_variation(args.input_file, args.output_dir, args.num_grammar)
