...
for exp_name in "${exp_names[@]}"; do
    for trial in $(seq 0 $(($NUM_TRIALS - 1))); do
        for data_dir in "$exp_base_dir"/"$exp_name"/*; do
            ...
            submit_job \
            train_transformer_small+"$data_name"+"$exp_name"+"$grammar_name"+trial"$trial" \
            gpu \
            --gpus=1 \
            --mem-per-cpu=16g \
            --tmp=20g \
            --time="$time_limit" \
            -- \
            bash neural_networks/train_and_evaluate_transformer_small.sh \
                "$data_dir" \
                "$RESULTS_DIR"/"$data_name"/"$exp_name"/transformer_small/"$grammar_name"_trial"$trial" \
