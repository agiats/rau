...
def sample_batch(args):
    ...
    grammar = instantiate(
        {"_target_": f"pcfg.{grammar_class}", "grammar_file": grammar_file}
    )
...
def monte_carlo_parallel(
    ...
    with tqdm.tqdm(total=n_samples, desc="Total Progress") as pbar:
        pbar.set_postfix(
            {"unique": 0, "samples/s": 0, "processes": n_processes, "batch": 0}
        )
        ...
        with mp.Pool(n_processes) as pool:
            ...
            while remaining_samples > 0:
                ...
                for i in range(n_processes):
                    process_batch_size = samples_per_process + (
                ...
                # バッチ処理の実行
                for process_id, samples, counts in pool.imap_unordered(
                    sample_batch, batch_args
                ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Parallel Monte Carlo sampling for PCFG"
    )
    ...
    parser.add_argument(
        "--n_samples", type=int, default=1_000_000, help="Number of samples"
    )
    parser.add_argument("--batch_size", type=int, default=10_000, help="Sub-batch size")
    parser.add_argument(
        "--max_expansions", type=int, default=100, help="Max number of expansions"
    )
    parser.add_argument(
        "--n_processes", type=int, default=None, help="Number of processes to use"
    )
    parser.add_argument(
        "--output_dir", type=str, default="results", help="Output directory"
    )
    parser.add_argument(
        "--output_name", type=str, default="samples", help="Output file name"
    )
