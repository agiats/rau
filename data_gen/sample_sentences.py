...
def sample_sentences(grammar_file, n, m, output_folder, bracketing):
    ...
    output_file = open(
        os.path.join(output_folder, "sample_" + grammar_name + ".txt"), "w"
    )
...
parser.add_argument(
    "-g", "--grammar_file", type=str, default="", help="Path to grammar file"
)
parser.add_argument(
    "-G",
    "--grammar_folder",
...
parser.add_argument(
    "-b", "--bracketing", type=bool, help="Include bracketing of constituents"
)
...
if args.grammar_file == "" and args.grammar_folder == "":
    ...
    print("Please provide either a single file OR a folder containing grammar files")
    ...
    grammar_files = [f for f in os.listdir(args.grammar_folder) if f.endswith(".gr")]
    for g in grammar_files:
        sample_sentences(
            os.path.join(args.grammar_folder, g),
            args.number_samples,
            args.max_expansions,
            args.output_folder,
            args.bracketing,
